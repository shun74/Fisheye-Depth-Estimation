...
class StereoVisionProcessor
{
  public:
    ...
    virtual void computeDisparity(const cv::Mat &left, const cv::Mat &right, cv::Mat &disparity) = 0;

    virtual void computePointCloud(const cv::Mat &left, const cv::Mat &right, cv::Mat &disparity, cv::OutputArray _pcd,
                                   cv::OutputArray _colors, std::vector<bool> &valid) = 0;
