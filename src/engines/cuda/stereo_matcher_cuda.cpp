...
namespace cuda
{
    ...
    void StereoMatcher::setParameters(config::StereoMatcherParams sm_p)
    {
        ...
        if (use_filter_) {
            disp_filter_ = cv::cuda::createDisparityBilateralFilter(sm_p.max_disp, sm_p.filter_size, sm_p.refine_iter);
            disp_filter_->setEdgeThreshold(sm_p.edge_thresh);
            disp_filter_->setMaxDiscThreshold(sm_p.disc_thresh);
            disp_filter_->setSigmaRange(sm_p.sigma_range);
        }
    }
