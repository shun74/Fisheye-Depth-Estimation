...
void computeFisheyeMap(cv::Mat K, cv::Mat D, cv::Mat R, cv::Mat P, cv::Size img_size, int dtype, cv::Mat &map_x,
                       cv::Mat &map_y)
{
    ...
    cv::fisheye::initUndistortRectifyMap(K, D, R, P, img_size, dtype, rect_map_x, rect_map_y);
    computeEquirectangleMaps(P, img_size, rect_to_eqrec_map_x, rect_to_eqrec_map_y);

    cv::remap(rect_map_x, map_x, rect_to_eqrec_map_x, rect_to_eqrec_map_y, cv::INTER_LINEAR, cv::BORDER_CONSTANT);
    cv::remap(rect_map_y, map_y, rect_to_eqrec_map_x, rect_to_eqrec_map_y, cv::INTER_LINEAR, cv::BORDER_CONSTANT);
}
...
void computeStereoRectifyMaps(cv::Mat K1, cv::Mat K2, cv::Mat D1, cv::Mat D2, cv::Mat R1, cv::Mat R2, cv::Mat P1,
                              ...
                              std::vector<cv::Mat> &right_maps, int dtype)
{
    computeFisheyeMap(K1, D1, R1, P1, img_size, dtype, left_maps[0], left_maps[1]);
    computeFisheyeMap(K2, D2, R2, P2, img_size, dtype, right_maps[0], right_maps[1]);
}
